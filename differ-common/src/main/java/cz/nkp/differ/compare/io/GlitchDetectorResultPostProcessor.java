...
public class GlitchDetectorResultPostProcessor implements ResultPostProcessor,
		RefreshableResultPostProcessor<GlitchDetectorConfig> {
	...
	private static final String[] colours = new String[] { "Max red ratio",
			"Max green ratio", "Max blue ratio" };

	private GlitchDetectorConfig config = new GlitchDetectorConfig();

	@Override
	public List<ImageMetadata> process(ImageProcessorResult result,
			GlitchDetectorConfig glitchDetectorConfig) {
		...
		if (grayscaleHistogram != null) {
			boolean valid = true;
			if (grayscaleHistogram[0] == resolution
					|| grayscaleHistogram[255] == resolution) {
				glitchDetected = true;
				valid = false;
			}
			ImageMetadata white = createImageMetadataEntry(
					"Number of absolute white pixels",
					Integer.toString(grayscaleHistogram[255]), valid,
					PIXEL_UNIT);
			metadata.add(white);
			ImageMetadata black = createImageMetadataEntry(
					"Number of absolute black pixels",
					Integer.toString(grayscaleHistogram[0]), valid, PIXEL_UNIT);
			metadata.add(black);
		}
		...
		if (colorHistogram != null) {
			for (int i = 0; i != 3; i++) {
				...
				switch (i) {
				case 0:
					maxAllowedRatio = glitchDetectorConfig
							.getMaxAllowedRatioForAbsoluteRed();
					break;
				case 1:
					maxAllowedRatio = glitchDetectorConfig
							.getMaxAllowedRatioForAbsoluteGreen();
					break;
				case 2:
					maxAllowedRatio = glitchDetectorConfig
							.getMaxAllowedRatioForAbsoluteBlue();
					break;
				}
				boolean valid = true;
				...
				ImageMetadata entry = createImageMetadataEntry(colours[i],
						Double.toString(ratio * 100), valid, PERCENT_UNIT);
				metadata.add(entry);
			}
		}
		...
		entry.setValue(new ValidatedProperty(Boolean.toString(glitchDetected),
				glitchDetected));
		metadata.add(entry);
		return metadata;
	}
