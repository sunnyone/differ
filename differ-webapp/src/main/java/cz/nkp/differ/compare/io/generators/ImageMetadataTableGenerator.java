...
public class ImageMetadataTableGenerator {
    ...
    public ImageMetadataTableGenerator(ImageProcessorResult[] results, CompareComponent parent) {
    ...
    public Layout getComponent() {
        ...
        List<ImageMetadata>[] dataArray = (List<ImageMetadata>[]) Array.newInstance(List.class, results.length + 1);
        ...
        layout.addComponent(generateMetadataTable("Identification", filterByProperties(metadata, metadataGroups.getIdentificationProperties())));
	layout.addComponent(generateMetadataTable("Validation", filterByProperties(metadata, metadataGroups.getValidationProperties())));
	layout.addComponent(generateMetadataTable("Characterization", filterByProperties(metadata, metadataGroups.getCharacterizationProperties())));
        if (!profileProps.isEmpty()) {
	    layout.addComponent(generateMetadataTable("JPEG2000 profile validation", profileProps));
	}
        layout.addComponent(generateMetadataTable("Others", filterByOtherProperties(metadata, metadataGroups.getAllProperties())));
        final GlitchDetectorConfig config = new GlitchDetectorConfig();
	if (!glitchProps.isEmpty()) {
	    final Table table = generateMetadataTable(GlitchDetectorResultPostProcessor.SOURCE_NAME, glitchProps);
		...
		public void buttonClick(Button.ClickEvent event) {
			...
			public void buttonClick(Button.ClickEvent event) {
			    ...
			    newTable = generateMetadataTable(GlitchDetectorResultPostProcessor.SOURCE_NAME, newGlitchProps);
    ...
    private HashMap<String, ComparedImagesMetadata> getMetadataTable(List<ImageMetadata>[] dataArray) {
    ...
    private Table generateMetadataTable(String group, Map<String, ComparedImagesMetadata> hashmap) {
        ...
        for (int i = 0; i!= results.length; i++) {
            ...
            metadataTable.addContainerProperty(name, SortableButton.class, null);
        ...
        metadataTable.addContainerProperty(COLUMN_UNIT_PROPERTY, String.class, null);
        metadataTable.addContainerProperty(COLUMN_CONFLICT_PROPERTY, Boolean.class, null);
        // rows
        int row = 0;
        ...
        metadataTable.setWidth(results.length * TABLE_WIDTH, Sizeable.UNITS_PIXELS);
        metadataTable.setPageLength(Math.min(row, 10));
        metadataTable.sort(new Object[]{ COLUMN_KEY_PROPERTY }, new boolean[] {true });
        if (group.equals("JPEG2000 profile validation") || group.equals(GlitchDetectorResultPostProcessor.SOURCE_NAME)) {
    ...
    private static class ConflictCellStyleGenerator implements Table.CellStyleGenerator {
        ...
        public String getStyle(Object itemId, Object propertyId) {
            if (propertyId == null) {
                Boolean conflict = (Boolean) metadataTable.getContainerProperty(itemId, COLUMN_CONFLICT_PROPERTY).getValue();
    ...
    private static class ValidatedPropertyCellStyleGenerator implements Table.CellStyleGenerator {
