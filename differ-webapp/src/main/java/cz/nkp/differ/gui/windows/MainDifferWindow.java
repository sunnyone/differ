...
public class MainDifferWindow extends Window implements LoginListener {
	...
	public MainDifferWindow() {
		super("The Image Data Validator - DIFFER");// Sets the title of the
													// application
		...
		if (DifferApplication.getCurrentApplication().getLoggedUser() != null) {
			...
			resultsTab = MainDifferWindow.createDynamicContentTab(results,
					"Results", menuTabs);
		}
		 ...
		 */
		addComponent(new ProjectHeaderPanel(this, loginContext));// Component
																	// that
																	// represents
																	// the
																	// top-page
	...
	private static Tab createStaticContentTab(String source, String caption,
			TabSheet parent) {
		try {
	...
	public void onLogin(User user) {
		...
		ResultManagerTab results = new ResultManagerTab(this, false);
		resultsTab = MainDifferWindow.createDynamicContentTab(results,
				"Results", menuTabs, POSITION_OF_RESULT_IN_TABS);
	}
